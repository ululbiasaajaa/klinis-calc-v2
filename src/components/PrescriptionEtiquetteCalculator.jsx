...
export default function PrescriptionEtiquetteCalculator() {
  ...
  // BACA INFO HOSIPITAL UNTUK KOP ETIKET
    ...
    <div className="space-y-6 text-xs">
      <div className={`p-5 rounded-2xl border ${isDark ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200 shadow-sm'}`}>
        ...
        {medications.length > 0 && (
          <div className="mb-4 p-3 rounded-xl bg-blue-950/40 border border-blue-800/50">
            <span className="text-[10px] text-blue-400 font-bold block mb-1">💡 Pilih dari Obat Aktif Pasien:</span>
            <div className="flex flex-wrap gap-1.5">
              {medications.map((m, idx) => (
                <button
                  ...
                  className="px-2.5 py-1 rounded-lg bg-slate-800 hover:bg-blue-600 text-white text-[10px] border border-slate-700 transition-all cursor-pointer"
                >
                  + {m.name}
                </button>
              ))}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <input
              ...
              className={`w-full p-2.5 rounded-xl border outline-none text-xs font-semibold ${isDark ? 'bg-slate-950 border-slate-800 text-white focus:border-blue-500' : 'bg-slate-50 border-slate-300 text-slate-900'}`}
            />
          </div>
        ...
        <div className="mt-6 flex justify-end">
          <button
            ...
            className="bg-emerald-600 hover:bg-emerald-500 text-white font-bold py-2.5 px-5 rounded-xl shadow-lg transition-all flex items-center gap-2 text-xs cursor-pointer"
          >
