...
export default function Sidebar({
  ...
  // Default kita buka kategori 'Utama' & 'Dosis & Obat'
  ...
  return (
    <aside
      className={`fixed md:static inset-y-0 left-0 z-40 w-64 p-4 transform transition-colors duration-300 ease-in-out flex flex-col justify-between select-none ${
        isDark 
      ...
      <div className="flex flex-col h-full overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto space-y-2 pr-1 custom-scrollbar">
          {categories.map((cat) => {
            ...
            if (itemsInCat.length === 0) return null; // Sembunyikan kategori jika kosong karena pencarian
            
            const isOpen = openCategories[cat] || searchQuery.trim() !== '';

            return (
              <div key={cat} className="space-y-1">
                {/* KATEGORI HEADER (ACCORDION TRIGGER) */}
                <button
